...
#line 14 "applications\\canapp.c"
#line 1 "drivers\\gpio.h"
...
#line 100 "applications\\canapp.c"
int can_bus_hook_init(void)
{
    rt_device_t candev;

    candev = rt_device_find("bxcan1");
    if (!(candev)) { rt_assert_handler("candev", __FUNCTION__, 105); };
    rt_device_control(candev, 0x19, (void *)can1_bus_hook);
#line 113 "applications\\canapp.c"
    return 0;
}
__attribute__((used)) const init_fn_t __rt_init_can_bus_hook_init __attribute__((section(".rti_fn." "3"))) = can_bus_hook_init;
...
{
    {(1), (0), (0), (0), (0xFFFFFFFF), -1, (can1ind), (&can_data[0]. event)},
    {(2), (0), (0), (0), (0xFFFFFFFF), -1, (can1ind), (&can_data[0]. event)},
    {(3), (0), (1), (0), (0xFFFFFFFF), -1, (can1ind), (&can_data[0]. event)},
    {(4), (0), (0), (1), (0xFFFFFFFF), -1, (can1ind), (&can_data[0]. event)},
};
struct rt_can_filter_item filter2item[4] =
{
    {(1), (0), (0), (0), (0xFFFFFFFF), -1, (can2ind), (&can_data[1]. event)},
    {(2), (0), (0), (0), (0xFFFFFFFF), -1, (can2ind), (&can_data[1]. event)},
    {(3), (0), (1), (0), (0xFFFFFFFF), -1, (can2ind), (&can_data[1]. event)},
    {(4), (0), (0), (1), (0xFFFFFFFF), -1, (can2ind), (&can_data[1]. event)},
};
...
void rt_can_thread_entry(void *parameter)
{
    ...
    if (!(candev)) { rt_assert_handler("candev", __FUNCTION__, 183); };
    rt_event_init(&canpara->event, canpara->name, 0x00);
    rt_device_open(candev, (0x003 | 0x100 | 0x400));
    rt_device_control(candev, 0x13, canpara->filter);
    while (1)
    {
        if (
...
int rt_can_app_init(void)
{
    ...
    tid = rt_thread_create("canapp1",
                           ...
                           512, 32 / 3 - 1, 20);
    if (tid != (0)) rt_thread_startup(tid);

#line 250 "applications\\canapp.c"

    return 0;
}

__attribute__((used)) const init_fn_t __rt_init_rt_can_app_init __attribute__((section(".rti_fn." "6"))) = rt_can_app_init;
